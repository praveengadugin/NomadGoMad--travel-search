...
app.get('/getNearByPlaces', function (request, response) {
  ...
  if(fromLocationOption == 'hereLocation') {
    ...
    var url = 'https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=' + currentLocationLatitude + ',' + currentLocationLongitude + '&radius=' + (distance * 1609.34) + '&type=' + category + '&keyword=' + keyword +'&key=AIzaSyD_xF2kfR8mDb4VlZ_MRfcRkKfm2RoZwSY';
    console.log(url);
    ...
    var url = 'https://maps.googleapis.com/maps/api/geocode/json?address=' + encodeURIComponent(customLocationText) +'&key=AIzaSyD_xF2kfR8mDb4VlZ_MRfcRkKfm2RoZwSY';
    var promise = new Promise(function(resolve,reject) {
      callApi(url, response, resolve, reject);
    });
    promise.catch(function(error) {
    ...
    promise.then(function(data) {
      ...
      if(responseJSON && responseJSON.results && responseJSON.results.length > 0) {
        ...
        url = 'https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=' + latitude + ',' + longitude + '&radius=' + (distance * 1609.34) + '&type=' + category + '&keyword=' + keyword +'&key=AIzaSyD_xF2kfR8mDb4VlZ_MRfcRkKfm2RoZwSY';
        promise.then(function(data) {
...
app.get('/getNext20Results', function (request, response) {
  ...
  var url = 'https://maps.googleapis.com/maps/api/place/nearbysearch/json?pagetoken=' + token +'&key=AIzaSyD_xF2kfR8mDb4VlZ_MRfcRkKfm2RoZwSY';
  var promise = new Promise(function(resolve,reject) {
    callApi(url, response, resolve, reject);
  });
...
app.get('/getPlaceDetails', function(request, response) {
  ...
  var url = 'https://maps.googleapis.com/maps/api/place/details/json?placeid=' + placeId +'&key=AIzaSyD_xF2kfR8mDb4VlZ_MRfcRkKfm2RoZwSY';
  console.log(url)
  var promise = new Promise(function(resolve,reject) {
    callApi(url, response, resolve, reject);
  });
...
app.get('/getDirections', function(request, response) {
  ...
  var url = 'https://maps.googleapis.com/maps/api/directions/json?origin=' + sourceLocation + '&destination=' + destinationPlaceId + '&mode=' + mode +'&key=AIzaSyD_xF2kfR8mDb4VlZ_MRfcRkKfm2RoZwSY'
  console.log('URL ' + url);
  var promise = new Promise(function(resolve,reject) {
    callApi(url, response, resolve, reject);
  });
